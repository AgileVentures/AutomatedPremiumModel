...
without_internet({
  test_that("fetch_history_from_slack_channel calls right endpoint", {
    expect_GET(fetch_history_from_slack_channel("C0285CSUF",test_token), paste("https://slack.com/api/channels.history?token=", test_token, "&channel=C0285CSUF&count=999", sep=""))
  })
  
  test_that("fetch_history_from_slack_channel calls right endpoint when there is a ts", {
    expect_GET(fetch_history_from_slack_channel("C0285CSUF", test_token, "123.12"), paste("https://slack.com/api/channels.history?token=", test_token, "&channel=C0285CSUF&count=999&latest=123.12", sep=""))
  })
})

with_mock_API({
  test_that("We can get history", {
    result <- fetch_history_from_slack_channel("C0285CSUF", test_token)
  ...
  test_that("we can get full history amid certain dates", {
    ...
    full_history <- fetch_history_from_slack_channel_over_period("C0285CSUF", three_sundays_ago, last_saturday, test_token)
  ...
  test_that("we can respect has_more value", {
    last_saturday <- as.Date("2017-10-07")
    three_sundays_ago <- as.Date("2017-10-07") - 20
    full_history <- fetch_history_from_slack_channel_over_period("C300", three_sundays_ago, last_saturday, test_token)
  ...
  test_that("message_project_channel_with_user_names calls right endpoint", {
    ...
    expect_GET(message_project_channel_with_user_names(users, channels, api_token),"https://slack.com/api/chat.postMessage?token=abc-d&channel=c2&username=premium-automated-bot&text=%3C!here%3E%20this%20week's%20picks%20for%20premium%20signup%20are:%20yada%20yolo")
  })
  ...
  test_that("message_admin_with_user_emails calls right endpoint", {
    ...
    expect_GET(message_admin_with_user_emails(targeted_users_emails, users, api_token), "https://slack.com/api/chat.postMessage?token=abc-d&channel=&username=premium-automated-bot&text=this%20week's%20picks'%20emails%20for%20premium%20signup%20are:%20c(2,%201)")
  })
})
